// Experimental generated audio RSP microcode for Castlevania: Legacy of Darkness.
// Generated with RSPRecomp from task->ucode dumped at RDRAM 0x800AC200.
// Config: text_offset=0, text_size=0xE20, text_address=0x1080, word-swapped dump.
// extra_indirect_branch_targets are LoD audio jump-table entries from task->ucode_data[0x10..0x2F].
// Postprocess: writes to RSP register zero emitted as invalid `0 = ...` are converted to discarded expressions.
// Keep gated behind LOD_USE_RECOMPILED_AUDIO_RSP until audio is validated.
...
static inline uint32_t lod_rsp_dma_len(uint32_t inclusive_len) {
    return inclusive_len + 1;
}

static inline uint32_t lod_rsp_count_nonzero_dmem(uint32_t dmem_addr, uint32_t len) {
    uint32_t nonzero = 0;
    for (uint32_t i = 0; i < len && dmem_addr + i < 0x1000; i++) {
        if (RSP_MEM_BU(i, dmem_addr) != 0) {
            nonzero++;
        }
    }
    return nonzero;
...
static inline LodRspDmemI16Summary lod_rsp_summarize_dmem_i16(uint32_t dmem_addr, uint32_t len) {
    LodRspDmemI16Summary summary{};
    if (dmem_addr >= 0x1000) {
        return summary;
    }

    const uint32_t max_len = 0x1000 - dmem_addr;
    if (len > max_len) {
        len = max_len;
    }
    len &= ~UINT32_C(1);
    summary.samples = len / 2;
    ...
    for (uint32_t off = 0; off < len; off += 2) {
        const int32_t sample = (int16_t)(uint16_t)RSP_MEM_HU_LOAD(off, dmem_addr);
...
static inline uint32_t lod_rsp_count_nonzero_rdram(uint8_t* rdram, uint32_t dram_addr, uint32_t len) {
    uint32_t nonzero = 0;
    const uint32_t phys = dram_addr & 0xFFFFF8;
    for (uint32_t i = 0; i < len && phys + i < 0x800000; i++) {
        (void)rdram;
        if (MEM_B(0, (int64_t)(int32_t)(phys + i + 0x80000000)) != 0) {
            nonzero++;
        }
    }
    return nonzero;
...
static inline void lod_rsp_dump_dmem_head(uint32_t dmem_addr, uint32_t len) {
    const uint32_t n = len < 16 ? len : 16;
    for (uint32_t i = 0; i < n && dmem_addr + i < 0x1000; i++) {
        fprintf(stderr, " %02X", RSP_MEM_BU(i, dmem_addr));
    }
}

static inline void lod_rsp_dump_rdram_range_n64(uint8_t* rdram, uint32_t addr, uint32_t size,
                                                const char* path) {
    (void)rdram;
    const uint32_t phys = addr & 0x7FFFFF;
    if (phys >= 0x800000 || size > 0x800000 - phys) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        uint8_t b = (uint8_t)MEM_B(0, (int64_t)(int32_t)(phys + i + 0x80000000));
        fwrite(&b, 1, 1, f);
    }
    fclose(f);
...
static inline void lod_rsp_trace_dmem_i16_range(const char* tag, uint32_t call, uint32_t dmem_addr,
                                                uint32_t len, uint32_t cmd_w0, uint32_t cmd_w1) {
    const LodRspDmemI16Summary summary = lod_rsp_summarize_dmem_i16(dmem_addr, len);
    if (call <= 20 || (summary.nonzero != 0 && (call <= 200 || (call % 60) == 0)) ||
        (call % 240) == 0) {
        fprintf(stderr,
                "[RSP_AUDIO_BUF] %s#%u dmem=0x%03" PRIX32 " len=0x%X"
                " i16_min=%" PRId32 " i16_max=%" PRId32 " i16_nonzero=%u/%u"
                " cmd=%08" PRIX32 " %08" PRIX32 " head:",
                tag, call, dmem_addr, len, summary.min, summary.max, summary.nonzero,
                summary.samples, cmd_w0, cmd_w1);
        const uint32_t head_samples = summary.samples < 8 ? summary.samples : 8;
        for (uint32_t i = 0; i < head_samples; i++) {
            const int32_t sample = (int16_t)(uint16_t)RSP_MEM_HU_LOAD(i * 2, dmem_addr);
            fprintf(stderr, " %" PRId32, sample);
        }
        fprintf(stderr, "\n");
    }
}
...
#define LOD_RSP_TRACE_DMEM_I16(tag, dmem_addr, len)                                             \
    do {                                                                                        \
        static uint32_t LOD_RSP_TRACE_CONCAT(lod_rsp_trace_dmem_i16_count_, __LINE__) = 0;      \
        uint32_t lod_rsp_trace_dmem_i16_call =                                                   \
            ++LOD_RSP_TRACE_CONCAT(lod_rsp_trace_dmem_i16_count_, __LINE__);                    \
        lod_rsp_trace_dmem_i16_range((tag), lod_rsp_trace_dmem_i16_call, (dmem_addr), (len),    \
                                     r25, r26);                                                  \
    } while (0)

static inline void lod_rsp_dma_read_trace(uint8_t* rdram, uint32_t dmem_addr, uint32_t dram_addr,
                                          uint32_t rd_len, int line) {
    static uint32_t read_count = 0;
    read_count++;
    const uint32_t len = lod_rsp_dma_len(rd_len);
    const uint32_t before_nonzero = lod_rsp_count_nonzero_rdram(rdram, dram_addr, len);
    dma_rdram_to_dmem(rdram, dmem_addr, dram_addr, rd_len);
    if (read_count <= 40 || (read_count % 500) == 0) {
        fprintf(stderr,
                "[RSP_AUDIO_DMA] read#%u line=%d dram=0x%08" PRIX32 " dmem=0x%03" PRIX32
                " len=0x%X rdram_nonzero=%u dmem_nonzero=%u head:",
                read_count, line, dram_addr, dmem_addr, len, before_nonzero,
                lod_rsp_count_nonzero_dmem(dmem_addr, len));
        lod_rsp_dump_dmem_head(dmem_addr, len);
        fprintf(stderr, "\n");
    }
}

static inline void lod_rsp_dma_write_trace(uint8_t* rdram, uint32_t dmem_addr, uint32_t dram_addr,
                                           uint32_t wr_len, int line) {
    static uint32_t write_count = 0;
    write_count++;
    const uint32_t len = lod_rsp_dma_len(wr_len);
    const uint32_t dmem_nonzero = lod_rsp_count_nonzero_dmem(dmem_addr, len);
    const LodRspDmemI16Summary summary = lod_rsp_summarize_dmem_i16(dmem_addr, len);
    if (write_count <= 80 || dmem_nonzero != 0 || (write_count % 500) == 0) {
        fprintf(stderr,
                "[RSP_AUDIO_DMA] write#%u line=%d dmem=0x%03" PRIX32 " dram=0x%08" PRIX32
                " len=0x%X dmem_nonzero=%u i16_min=%" PRId32 " i16_max=%" PRId32
                " i16_nonzero=%u/%u head:",
                write_count, line, dmem_addr, dram_addr, len, dmem_nonzero,
                summary.min, summary.max, summary.nonzero, summary.samples);
        lod_rsp_dump_dmem_head(dmem_addr, len);
        fprintf(stderr, "\n");
    }
    dma_dmem_to_rdram(rdram, dmem_addr, dram_addr, wr_len);
}

static inline void lod_rsp_trace_task_start(uint8_t* rdram, uint32_t ucode_addr) {
    static uint32_t task_count = 0;
    task_count++;
    const OSTask* task = reinterpret_cast<const OSTask*>(dmem + 0xFC0);
    if (task_count <= 20 || (task_count % 120) == 0) {
        uint32_t op_counts[16] = {};
        uint32_t op_other = 0;
        const uint32_t alist_phys = task->t.data_ptr & 0x7FFFFF;
        if (alist_phys < 0x800000) {
            const uint32_t cmd_bytes = task->t.data_size & ~UINT32_C(7);
            for (uint32_t off = 0; off < cmd_bytes && alist_phys + off < 0x800000; off += 8) {
                uint8_t op = (uint8_t)MEM_B(0, (int64_t)(int32_t)(alist_phys + off + 0x80000000));
                if (op < 16) {
                    op_counts[op]++;
                } else {
                    op_other++;
        ...
        fprintf(stderr,
                ...
                " data_ptr=0x%08" PRIX32 "/0x%X out=0x%08" PRIX32 " out_size_ptr=0x%08" PRIX32
                " ucode_data=0x%08" PRIX32 "/0x%X alist_nonzero=%u"
                " ops[0..15]=%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u other=%u\n",
                task_count, ucode_addr, task->t.type, task->t.flags,
                task->t.data_ptr, task->t.data_size,
                task->t.output_buff, task->t.output_buff_size,
                task->t.ucode_data, task->t.ucode_data_size,
                lod_rsp_count_nonzero_rdram(rdram, task->t.data_ptr, task->t.data_size),
                op_counts[0], op_counts[1], op_counts[2], op_counts[3],
                op_counts[4], op_counts[5], op_counts[6], op_counts[7],
                op_counts[8], op_counts[9], op_counts[10], op_counts[11],
                op_counts[12], op_counts[13], op_counts[14], op_counts[15],
                op_other);
    }
    if (task_count == 1 || task_count == 600 || task_count == 840 ||
        task_count == 960 || task_count == 1080) {
        char path[128];
        snprintf(path, sizeof(path), "/tmp/lod_aspMain_alist_task%04u.bin", task_count);
        lod_rsp_dump_rdram_range_n64(rdram, task->t.data_ptr,
                                     task->t.data_size < 0x4000 ? task->t.data_size : 0x4000,
                                     path);
    }
}
...
#undef DO_DMA_READ
#undef DO_DMA_WRITE
#define DO_DMA_READ(rd_len) lod_rsp_dma_read_trace(rdram, dma_mem_address, dma_dram_address, (rd_len), __LINE__)
#define DO_DMA_WRITE(wr_len) lod_rsp_dma_write_trace(rdram, dma_mem_address, dma_dram_address, (wr_len), __LINE__)
#endif
...
RspExitReason aspMain(uint8_t* rdram, [[maybe_unused]] uint32_t ucode_addr) {
    ...
    RSP rsp{};
#if LOD_ENABLE_RSP_AUDIO_TRACE
    lod_rsp_trace_task_start(rdram, ucode_addr);
#endif
    r1 = 0xFC0;
    // addi        $24, $zero, 0x360
...
L_13A8:
...
#if LOD_ENABLE_RSP_AUDIO_TRACE
    {
        ...
        LOD_RSP_TRACE_DMEM_I16("interleave.left", ((r25 >> 16) & 0xFFFF) + 0x5C0,
                               lod_count);
        LOD_RSP_TRACE_DMEM_I16("interleave.right", (r25 & 0xFFFF) + 0x5C0,
                               lod_count);
        LOD_RSP_TRACE_DMEM_I16("interleave.out", RSP_MEM_HU_LOAD(0X2, r24),
                               lod_count * 2);
    }
#endif
    goto L_1118;
...
#if LOD_ENABLE_RSP_AUDIO_TRACE
    {
        const uint32_t lod_out = RSP_MEM_HU_LOAD(0X2, r24);
        const uint32_t lod_count = RSP_MEM_HU_LOAD(0X4, r24);
        LOD_RSP_TRACE_DMEM_I16("adpcm.out.base", lod_out, lod_count + 0x20);
        LOD_RSP_TRACE_DMEM_I16("adpcm.out.decoded", lod_out + 0x20, lod_count);
    }
#endif
    goto L_1118;
    // mtc0        $zero, SP_SEMAPHORE
...
L_1B24:
    ...
    {
        const uint32_t lod_count = RSP_MEM_HU_LOAD(0X4, r24);
        LOD_RSP_TRACE_DMEM_I16("resample.in", RSP_MEM_HU_LOAD(0X0, r24), lod_count + 0x20);
        LOD_RSP_TRACE_DMEM_I16("resample.out", RSP_MEM_HU_LOAD(0X2, r24), lod_count);
    }
#endif
    goto L_1118;
    // mtc0        $zero, SP_SEMAPHORE
...
L_1E10:
...
#if LOD_ENABLE_RSP_AUDIO_TRACE
    {
        ...
        LOD_RSP_TRACE_DMEM_I16("envmixer.aux1", RSP_MEM_HU_LOAD(0XC, r24), lod_count);
        LOD_RSP_TRACE_DMEM_I16("envmixer.aux2", RSP_MEM_HU_LOAD(0XE, r24), lod_count);
    }
#endif
    goto L_1118;
    // mtc0        $zero, SP_SEMAPHORE
...
L_1E94:
    ...
    {
        const uint32_t lod_count = RSP_MEM_HU_LOAD(0X4, r24);
        LOD_RSP_TRACE_DMEM_I16("mixer.dst", (r25 & 0xFFFF) + 0x5C0, lod_count);
        LOD_RSP_TRACE_DMEM_I16("mixer.src", ((r25 >> 16) & 0xFFFF) + 0x5C0, lod_count);
    }
#endif
    goto L_1118;
    // nop
