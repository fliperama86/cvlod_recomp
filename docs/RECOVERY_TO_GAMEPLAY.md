...
|----|------|------------|----------|-------------|
| RENDER-001 | intro/prologue/merchant geometry explosions, black models, widescreen edge popping | default build and diagnostic A/B build compiled on `diag/render-glitch-root-causes` | Added default-off RT64 diagnostics and toggles for three renderer-address hypotheses: direct `0x86xxxxxx` display-list operands bypassing segment-6 lookup, RDP/RSP physical-data stripping of LoD NI `0x8E`/`0x8F` mirrors to empty `0x0E`/`0x0F`, and global `f3dex.forceBranch` selecting alternate BranchZ/W display-list paths. Added throttled invalid-sub-DL guard logging. Widescreen edge popping still looks more likely to be game-side 4:3 CPU/object culling than RT64 geometry corruption. | Run manual A/B from the affected cutscenes with one toggle at a time: first `LOD_FIX_DIRECT_SEGMENT_86_ADDRS`, then `LOD_FIX_NI_RDP_EXTENDED_ADDRS` + `LOD_FIX_NI_RSP_EXTENDED_ADDRS`, then `LOD_DISABLE_RT64_FORCE_BRANCH`; enable `LOD_ENABLE_RENDER_ADDR_TRACE` only for short capture windows because it is intentionally throttled but still noisy. |
