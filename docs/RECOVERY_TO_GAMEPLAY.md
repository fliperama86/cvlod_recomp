...
|----|------|------------|----------|-------------|
| RENDER-001 | intro/prologue/merchant geometry explosions, black models, widescreen edge popping | default build and diagnostic A/B build compiled on `diag/render-glitch-root-causes`; user manual all-toggle test | Added default-off RT64 diagnostics and toggles for three renderer-address hypotheses: direct `0x86xxxxxx` display-list operands bypassing segment-6 lookup, RDP/RSP physical-data stripping of LoD NI `0x8E`/`0x8F` mirrors to empty `0x0E`/`0x0F`, and global `f3dex.forceBranch` selecting alternate BranchZ/W display-list paths. In the all-toggle diagnostic build, geometry explosions and black/missing models appeared unchanged, while widescreen edge popping seemed reduced. This weakens the address-mapping hypotheses for the model corruption and makes `LOD_DISABLE_RT64_FORCE_BRANCH` the active widescreen-popping lead. | Validate with `build-render-no-forcebranch` only. If popping improvement holds, consider forceBranch policy/BranchZ-W behavior separately. For remaining geometry/black-model glitches, pivot to RT64 F3DEX vertex/matrix/lighting/combiner handling and unsupported display-list command paths rather than raw segmented address mapping. |
