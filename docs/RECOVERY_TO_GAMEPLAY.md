...
- Windows issue #7/#21 crackle evidence: reporter logs used SDL `directsound` with a 44.1 kHz/8-channel default device while LoD requested 48 kHz/F32/stereo. Linux PipeWire has also been reported as a possible audio-quality problem. The baseline now stores an SDL audio-driver preference in `audio.json`: `auto` selects WASAPI on Windows, PulseAudio on Linux, and the platform default elsewhere; explicit platform-supported choices are exposed under Settings -> Sound and require restart. A non-empty `SDL_AUDIODRIVER` environment variable still overrides `auto` for diagnostics, except the known-bad Windows DirectSound case is recovered to WASAPI. SUPERNOVA smoke build/run logged `SDL audio driver: wasapi`, 48 kHz stereo default/output, and no stale process; issue #21 still needs reporter confirmation that crackle is fixed again.
...
## Tower of Sorcery Black Screen (2026-06-22)

Status: partially validated. The black-screen/progression failure is user-confirmed fixed after the Tower map-overlay boundary splits plus the CPU-side segment-6 alias fix. The later Tower item/save interaction failure has been traced to scene graph pool exhaustion during item-name popup allocation, especially the 64-byte scene-data pool. A user A/B run completed Tower at Original refresh, while Display/unlocked refresh reproduced the stuck textbox path more easily.

What changed:

- Removed the old `map_ovl_29_func_802E4E9C` and `map_ovl_29_func_802E534C` stubs from `recomp.toml`.
- Extended `map_ovl_29`'s text span from `0x4F28` to `0x4FA0` and added `map_ovl_29_update_distance_flag` at `0x802E8A98`.
- Renamed `func_80147D5C` to `vec3_distance`.
...
- Added `LOD_FIX_PAIR126_INPUT_RELEASE`, default ON, to release NI pair-126 transition paths from the temporary `0x20000000` execution-lock flags when the state-init route skips the normal fade/destroy cleanup or when the destroy route leaves only the transition lock set.
...
- Added default-off `LOD_ENABLE_INTERACT_TRACE` instrumentation for common interactable selection, gated to Tower map ROM `0x007D9790`.
- Added `LOD_FIX_EXPAND_SCENE_NODE_POOL`, default ON, to raise centralized scene graph pool minimums before scene initialization. Current defaults are node pool `0x2A0` entries and 64-byte scene-data pool `0x480` entries, replacing the earlier node-only `0x300` attempt.
- Added `experimental_display_modes` to graphics settings, default Off. When Off, Aspect Ratio, HUD Placement, and Refresh Rate are locked to Original. The Zelda graphics menu still shows the advanced choices, but greys them until the user enables the warning-gated Experimental Display Modes toggle.

Evidence:

- Issue #18 save route loads Tower map overlay ROM `0x007D9790`, then remains black while music continues and NI pair `126`/`18` activity repeats.
- The newly generated Tower helper compares object `+0x50` against the current object/player pointer from `0x801CAC20`; when the distance gate fails, it stores zero at object `+0x74`.
- Pair `126` is a Tower-specific scene/controller overlay with a state dispatch table at `0x0F000A88`; the Tower table entry is index `9` at `0x0F0008B0`.
- User A/B test with the cutoff widened still produced the same black screen, so the current blocker is not the simple `240.0f` distance gate.
- Latest trace build loads Tower map overlay ROM `0x007D9790` and installs pair-`126` wrappers. Pair `126` advances through `state_init_0F000070`, `state_fade_in_0F000318`, `state_wait_0F000490`, `state_fade_out_0F000590`, and `state_destroy_0F00064C`; it reaches `frame=91`, sets `done=1`, clears its child pointer, then pair `18` continues looping. This rules out pair `126` being stuck as the direct black-screen cause.
- `osViBlack` is not re-enabled after the Tower transition; the last Tower-adjacent calls keep `active=0`.
- CFB snapshots prove the black output is in the game framebuffer itself. A pre-Tower `gs=3` snapshot (`/private/tmp/lod_cfb_gs3_us9773_dl5012_cfb225800.ppm`) has nonzero RGB data, while Tower snapshots immediately after map load, after pair-`126` destroy, and later in the same black screen are all pure RGB zero (`/private/tmp/lod_cfb_gs3_us10357_dl5304_cfb24B000.ppm`, `/private/tmp/lod_cfb_gs3_us10627_dl5439_cfb24B000.ppm`, `/private/tmp/lod_cfb_gs3_us12727_dl6284_cfb225800.ppm`).
- `LOD_ENABLE_TOWER_RENDER_SUMMARY=ON` shows Tower is not missing display-list submission. After map ROM `0x007D9790` loads, RT64 continues receiving HLE tasks with real scene work: roughly `121-124` draw calls, `1789-1793` indexed triangles, `3313-3321` vertices, `196-198` load operations, no GBI misses, and no command warnings. The black screen therefore looks like valid Tower geometry being rendered black or covered by a black pass, not a no-geometry/no-GBI condition.
- User restart/death A/B before the map-boundary split: controls responded immediately with `LOD_FIX_TOWER_INPUT_RELEASE`, flicker was gone, but floating crystal enemies were still missing.
- A conservative scan across all 47 map overlays found hidden stack-prologue-after-`jr ra` boundaries only in `map_ovl_29`; `python3 tools/audit_map_hidden_boundaries.py --fail-on-missing` now passes after the Tower splits.
- User validation after the map-boundary split plus CPU segment-6 alias fix: Tower can now be progressed through without the black-screen blocker or later segment-6 crash.
- The remaining interaction symptom uses shared common code around `Player_getActorCurrentlyInteractingWith` and `playerCanInteractWithInteractuable`; Tower-only traces should show whether object ID `0x27` candidates are missing, rejected by the position/facing gate, or selected but never flagged.
- First `LOD_ENABLE_INTERACT_TRACE` build crashed in the trace helper because guest object/player pointers were cast to `uint32_t` and then dereferenced through the recompiler `MEM_*` macros, losing KSEG0 sign extension. The trace helper now uses bounded safe RDRAM reads and re-sign-extends guest addresses before reading fields.
- Online/manual reference check found no Tower-specific item lockout mechanic. The original mechanic is Right C / right camera for floor item pickup, and Tower walkthroughs explicitly use save crystals and item crystals. Current trace focus is therefore implementation/state, not a known intended Tower rule.
- Verbose `LOD_ENABLE_INTERACT_TRACE` now logs all Tower candidate misses, actor/player XYZ deltas, trigger size, yaw, Controller Pak file number, and downstream `Interactable_*` state transitions (`main-flagged`, `init-check-entry`, `select-option-entry`, `stop-check-entry`, `destroy-entry`, `stop-interaction`).
- The failed-textbox pattern is now explained: object ID `0x27` interaction still succeeds and `commonTextboxAddItem` starts, but MFDS child object `0x0186` fails its scene-node/data allocation. Early captures showed the `0xB8` scene-node pool at `509/512` used. After node-pool expansion, later captures showed scene nodes still had headroom but the 64-byte scene-data pool reached `used=1152 free=0`, producing partial textbox allocation such as a missing border/texture and then a stuck common textbox. The popup stores `obj+0x24 = 0`, `func_8007E680` waits forever for it to become nonzero, and later pickups/save crystals only see `busy26=2`.
- 2026-06-25 WIP validation: parent branch `wip/ni-mips-fallback-prototype` with RT64 commit `6ecc770` fixes the issue #19 cutscene/merchant route. After copying Tower-of-Sorcery save slot #1 from `castlevania2.n64.us_tos.bin` into slot #3 of `castlevania2.n64.us.bin`, user validation also reported Tower of Sorcery good with no crashes. Earlier black-screen observation is now treated as save/profile mismatch rather than confirmed WIP regression.
...
- Proper fix follow-up, 2026-06-25: RT64 now uses a shared owner-aware LoD NI display-list resolver for `G_DL`, BranchZ, and BranchW. The resolver decodes `0x0E/0x0F/0x8E/0x8F` targets, accepts live current-pair bytes only when the target passes the same bounded structural display-list validation used for stale candidates, then searches recent saved NI pair snapshots for structurally valid fallback DLs. Diagnostic validation `/tmp/lod_recomp_logs/issue20_owner_resolver_20260625_223518.log` timed out cleanly, repeatedly resolving `G_DL 0x0F003E80` from live pair `30` to saved pair `26`. A later manual default `build/` route reproduced the `RSP::setVertex` OOB crash until live-current targets were tightened from a first-opcode/MIPS screen to full structural validation; after that stricter rule, the user retested the same problematic area and reported no crash.
...
- The init route is intentionally constrained to pair `126`, root state `count=1/index=0`, nonzero valid table entry, no fade child/link, no timer/limit/frame/done state, and the exact transition-lock exec flags. This covers issue #26 without map-specific checks.
...
- Issue #26 reporter save reproduced the same transition-lock class on the Outer Walls elevator as Henry, but `pair126.destroy` never fired. Pair `126` init selected entry `14`, found no fade child/link, and left exec at `0x20000000`; after the generalized init-no-fade release, user validation confirmed the black screen was fixed. Local trace `/tmp/lod_issue26/issue26_pair126_init_nofade_20260702_105715.log` shows `[PAIR126_INPUT_RELEASE_FIX] pair126.init-no-fade #2 exec 0x20000000 -> 0x38000000 map#4 map=0x0082E330` and subsequent non-black CFB samples.
...
| `LOD_FIX_RUN_DL_STALE_NI_FALLBACK` | **on** | General stale-NI render fix: shared owner-aware RT64 resolver validates live and stale `G_DL`/BranchZ/BranchW NI targets structurally, then recovers display lists from recent NI overlay snapshots when the current pair no longer owns the bytes | Keep on; disable only for A/B regressions in intro/model rendering |
...
| `LOD_FIX_NI_SEG6_CPU_ALIAS` | **on** | Validated CPU-side segment-6 alias fix; mirrors active NI segment data into CPU-visible `0x06xxxxxx` so KSEG0-mirrored segmented pointers do not crash when read by recompiled CPU code | Keep on; use `=0` only for A/B regression of the Tower segment-6 crash |
...
| `LOD_FIX_EXPAND_SCENE_NODE_POOL` | **on** | Raises centralized scene graph pool minimums to node `0x2A0` and scene-data `0x480` entries so dense Tower scenes leave headroom for transient item/textbox UI nodes | Keep on; use `=0` only to reproduce the Tower failed-item-textbox pool exhaustion |
| `LOD_ENABLE_INTERACT_TRACE` | off | Debug-only Tower-gated trace for common interactable selection, including save points and drops using object ID `0x27` and MFDS popup allocation state | Enable only for targeted manual Tower save/drop interaction repros |
...
| RENDER-006 | NI overlay mutable-data persistence fixes skirt deformation | `build-skirt-compute/LodRecomp`, local bounded 20s runs: `/tmp/lod_render_skirt_compute_detail_20s.log`, `/tmp/lod_render_skirt_anchor_defer2_20s.log`, `/tmp/lod_render_ni_persist_skirt_20s.log`; user visual validation; isolated `v0.2.3` retest `/tmp/lod_recomp_logs/v023_vertexfix_20260624_154625.log` | Confirmed CPU-side dynamic vertex generation, not RT64 drawing, drives the skirt cluster. `model_compute_vertex_delta` feeds `model_apply_scaled_vertex_delta_cmds`/`model_apply_vertex_delta_cmd`; the skirt cmdlist `0x0F002190` ping-pongs `0x0037AB30..0x0037AF20`. Skip/defer anchor tests reduced individual huge deltas but broke animation or reintroduced the bad frame, so anchor suppression was rejected. Root cause is broader: pair 53's writable NI overlay static data at `0x0F0023BC` (previous position used by `ni_ovl_053_func_0F00087C`) was reset to pristine ROM whenever pair 104 swapped into the shared 0x8F mirror. Generic per-pair NI segment save/restore (`LOD_FIX_NI_PERSIST_OVERLAY_DATA`, default ON) preserves mutable overlay data across pair swaps; logs show later `s0` deltas shrink from roughly `(1202,18,-300)` to frame-local values such as `(0.047,0,4.29)`, and user confirmed the skirt visual is fixed. A later isolated `v0.2.3` merchant-route retest did not fix the merchant vertex explosion, so the merchant glitch is separate from this skirt fix. | Keep the generic NI persistence fix. Revisit the merchant vertex explosion separately later, without mixing it into the issue #19 cutscene-crash regression. |
| RENDER-007 | Issue #19 cutscene RT64 crash | GitHub issue #19 reporter log; clean current repro `/tmp/lod_recomp_logs/current_clean_regression_20260624_153003.log`; isolated worktree runs `/tmp/lod_recomp_logs/v0210_regression_20260624_153652.log` and `/tmp/lod_recomp_logs/v023_vertexfix_20260624_154625.log` | This is a cutscene crash report, not a Tower item/save interaction bug. Do not classify it by the noisy `[INTERACT_TRACE]` lines that happened to be enabled in a diagnostic build. Clean current `v0.2.19` repro crashes after `MAP_OVL #4 loaded rom=0x78A020 size=0x92B0` in `RT64::Interpreter::processDisplayLists` with fault offset `0x8F008000` / phys `0x0F008000`, past 8MB RDRAM. Local RT64 symptom-guard experiments were shelved and should not be stacked further. Isolated `v0.2.10` did not crash on the route, and exact vertex-fix `v0.2.3` also did not crash, so the issue #19 crash regressed after `v0.2.10` and before or at current `v0.2.19`. | Keep this track focused on regression isolation for the cutscene crash, not item/save interaction and not the merchant vertex glitch. Next step is a methodical commit or tag bisect between `v0.2.10` and `v0.2.19`, with one validated build at a time. |
| RENDER-008 | Issue #19 NI display-list code-target prototype | Isolated `v0.2.16` prototype logs `/tmp/lod_recomp_logs/v0216_dl_source_trace_20260625_164255.log`, `/tmp/lod_recomp_logs/v0216_ni_entry_trace_20260625_181818.log`, `/tmp/lod_recomp_logs/v0216_ni_mips_fallback_20260625_185311.log`; main WIP `build/` log `/tmp/lod_recomp_logs/build_wip_ni_mips_fallback_20260625_192240.log`; branches `wip/ni-mips-fallback-prototype` and `wip/lod-ni-mips-fallback-prototype` | Exact trace showed the issue #19 crash path enters a `G_DL` from callers `0x001B4F88`/`0x001BEF88` to segmented target `0x0F000D58`, mapped as current `0x8F000D58`. That current target contains MIPS overlay code, not GBI display-list data, and RT64 accepted it only because first opcode byte `0x02` was present in the active GBI map. Prototype RT64 commit `6ecc770` rejects current `0x8E/0x8F` targets that look like MIPS code and tries stale NI display-list fallback. User validated issue #19 as good on the main WIP build, and the v0.2.16 prototype also removed the merchant vertex explosion seen on that route. After copying Tower-of-Sorcery slot #1 from `castlevania2.n64.us_tos.bin` to slot #3 of `castlevania2.n64.us.bin`, user also validated Tower of Sorcery as good with no crashes. Earlier Tower black-screen observation was caused by testing with the wrong save/profile and is not currently a confirmed WIP regression. | Promote from WIP only after one more clean validation build from `~/Projects/recomp/lod/build` using the intended portable/app-support save profile. If it remains good, merge the RT64 submodule commit and parent pointer deliberately. |
| RENDER-009 | Issue #20 owner-aware NI display-list resolver | `build-issue20-roottrace` diagnostic run `/tmp/lod_recomp_logs/issue20_owner_resolver_20260625_223518.log`; default `build/LodRecomp` manual retest | The Renon/merchant route is not a recompilation function-boundary problem. The crash occurs when RT64 follows a stale NI segmented display-list pointer and parses MIPS overlay code as GBI. A shared RT64 resolver now handles `G_DL`, BranchZ, and BranchW through the same LoD NI ownership model. The first shared version still accepted live current-pair targets too loosely by checking only the first opcode and MIPS-like words, which allowed a later `RSP::setVertex` OOB crash. Tightening live targets to the same bounded structural DL validation used for stale snapshot candidates removed the crash in the user's manual route. | Keep this as the general rule: do not accept any NI display-list target, live or stale, unless it is structurally a bounded display list. If this class regresses, inspect owner pair and structural-validation evidence before adding pair or scene-specific guards. |
| RENDER-010 | Issue #24 Villa/Gilles cutscene malformed display-list path | `build/LodRecomp` with focused `LOD_ENABLE_DL_PATH_TRACE=ON` diagnostics, then diagnostics-off rebuild; user manual validation | The cutscene/boss route entered a normal RDRAM `G_DL 0x00380000` from roots `0x001B42C0`/`0x001BE2C0`. The first command byte was accepted by the active GBI map, but the bounded path trace showed the target quickly became non-GBI data and reached an invalid F3DEX2 vertex command at `0x00380100` (`w0=0x013BB764`, count `187`, destination underflow). A general RT64 safety guard now validates `G_DL` targets with a bounded opcode-prefix scan, aborts malformed F3DEX2 vertex commands before OOB access, and clears the HLE display-list return stack after malformed aborts so one bad sublist cannot poison later roots. A first validator iteration incorrectly treated stale NI snapshot pointers as live-RDRAM OOB and dropped known intro skirt roots `0x0F002230`/`0x0F0022A8`; pointer classification was corrected to apply RDRAM bounds only to pointers actually inside the RDRAM allocation. User then confirmed the issue #24 path and intro skirt were both good. | Keep the fix generic and flag-gated (`LOD_FIX_RUN_DL_STRUCTURAL_PREFIX`, `LOD_FIX_INVALID_F3DEX2_VERTEX_ABORT`, `LOD_FIX_MALFORMED_DL_CLEAR_STACK`). If regressions appear, first check whether the skipped target is live RDRAM or a stale NI snapshot and inspect the rejected prefix, do not add scene-specific skips. |
...
| ENEMY-003 | Generalized NI call-table internal-target splitting | `tools/audit_indirect_targets.py --scan-call-tables --fail-on-missing`; `./tools/regen_recomp.sh`; `cmake -S . -B build-enemy-trace && cmake --build build-enemy-trace --target LodRecomp`; bounded 20s smoke `/tmp/lod_generalized_smoke.log`; user manual Stage 2 skull validation | Extended the NI indirect-target audit to locally slice MIPS `jalr` call-table patterns, so raw-data callback/state tables are audited without enabling noisy full-raw pointer scanning. Applied 166 build-safe function-boundary splits in `castlevania2.syms.toml`; regenerated code reaches the normal overlay_system truncation/repair point and the strengthened audit reports no missing targets. User confirmed the Stage 2 skull-head enemy behavior is also fixed by this generalized build, so at least one additional enemy bug beyond the Art Tower knights was covered by the preventive splits. `ni118` tiny branch labels and `ni120` result labels are excluded because they make N64Recomp abort early; `ni120` remains covered by its scoped runtime internal-label shim. | Keep regen step 10 running `--scan-call-tables --fail-on-missing`. If future call-table targets appear, run `--fix-syms`, regenerate, and only add an exclusion when a split demonstrably breaks N64Recomp or is already handled by a scoped shim. |
| ENEMY-004 | Generalized object callback and render guard for boss death paths | `build/LodRecomp` non-trace default build; user manual boss-kill validation on the Desktop save route | A boss-kill crash path involved invalid object callback targets and malformed/stale render commands during death effects. The default-on `LOD_FIX_BAD_OBJECT_CALLBACK` guard now skips object `+0x44` callbacks whose target is not a currently loaded recompiled function. RT64 display-list and RDP load guards now reject invalid NI branch targets and malformed texture/TLUT loads instead of dereferencing or looping on them. User confirmed the boss route no longer crashes; the visible vertex burst is the boss death animation, not a regression. | Keep the object callback guard generic and default-on. Use `[OBJECT_CB_GUARD]`, `[RT64-DL]`, and `[RDP_LOAD_GUARD]` logs only for future targeted repros. |
| ENEMY-005 | Tower death/reload/attack graphics-node crash | User crash logs `/tmp/lod_recomp_logs/tower_interact_trace.log` and `/tmp/lod_recomp_logs/mfds_child_trace.log`; static backtrace analysis | After dying and attacking/reloading in Tower of Sorcery, the draw path crashes in `gfx_emit_node_recursive` (`0x800070F0`) on stale segment-6-like graphics-node pointers. The first captured form was direct `0x0603D70C` (`rdram+0x8603D70C`); the later capture was guest `0x4603DFBC` (`rdram+0xC603DFBC`), the same low segment-6 offset through a kuseg/TLB alias. Both are outside the earlier active-NI-overlay segment-6 mirror and appear to be stale or not-yet-populated graphics-node links after map reload. | `LOD_FIX_SEG6_CPU_ALIAS_GUARD` now maps zero-filled 16MB segment-6 CPU aliases for both `0x06xxxxxx` and `0x46xxxxxx`, while preserving normal `osMapTLB` and NI copies over real pages. Validate with the user's Tower route. |
| ENEMY-006 | Issue #22 Dracula second-form crash during specific attack animation | User crash log with repeated `ni_ovl_013_func_0F0049AC` frames; `./tools/regen_recomp.sh`; `python3 tools/audit_indirect_targets.py --section 13 --scan-call-tables --target 0x0F004AC0`; `cmake --build build --target LodRecomp -j8`; user repro pass with no crash | Pair 13 explicitly calls internal target `0x0F004AC0` from `ni_ovl_013_func_0F0049AC` (`lui/addiu $t9, 0x0F004AC0; jalr $t9`). Without a symbol boundary, `LOOKUP_FUNC(0x0F004AC0)` range-fell back to the containing `0x0F0049AC` function, recursively re-entering the attack helper until a bad host/RDRAM address crash. Split `ni_ovl_013_func_0F0049AC` at `0x0F004AC0`, updated `symbol_addrs.txt`, regenerated, and the focused plus global indirect audits pass. Follow-up: `tools/audit_indirect_targets.py --scan-call-tables` now also detects constant `jalr` targets like this pattern. Current broader const-jalr candidates are explicitly excluded because splitting them as a batch currently breaks generation/linking. | Specific crash validated fixed. Revisit excluded const-jalr candidates only with generator/fixer support or a focused crash that proves an individual split is needed. |
...
## Current Baseline
- Tower of Sorcery interactable regression is currently observed only in this level so far. Latest trace shows the trigger is scene graph pool exhaustion during transient item-name textbox creation, not failed item selection or an intended Tower mechanic. Display/unlocked refresh worsens the failure path, so advanced aspect/refresh modes are now warning-gated behind `experimental_display_modes`.
