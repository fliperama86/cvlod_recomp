...
| ENEMY-006 | Issue #22 Dracula second-form crash during specific attack animation | User crash log with repeated `ni_ovl_013_func_0F0049AC` frames; `./tools/regen_recomp.sh`; `python3 tools/audit_indirect_targets.py --section 13 --scan-call-tables --target 0x0F004AC0`; `cmake --build build --target LodRecomp -j8` | Pair 13 explicitly calls internal target `0x0F004AC0` from `ni_ovl_013_func_0F0049AC` (`lui/addiu $t9, 0x0F004AC0; jalr $t9`). Without a symbol boundary, `LOOKUP_FUNC(0x0F004AC0)` range-fell back to the containing `0x0F0049AC` function, recursively re-entering the attack helper until a bad host/RDRAM address crash. Split `ni_ovl_013_func_0F0049AC` at `0x0F004AC0`, updated `symbol_addrs.txt`, regenerated, and the focused plus global call-table audits pass. | Validate manually with the issue #22 save, preferably with `LOD_CHEAT_INFINITE_HEALTH=1` only to survive until the problematic attack. |
