...
3. Improve KSEG0/PI evidence before changing the guard model: add targeted PI DMA destination logging or equivalent downstream evidence to prove whether any path is writing/reading `0x80810000`/past-RDRAM addresses after DMAMgr itself reports valid active/current RAM.
...
Work item `G6-001`: preserve the now-validated low-resolution natural route into gameplay, then stabilize long-idle gameplay and high-resolution intro/gameplay `gs=3`. The NI TLB skip-window bug and `0x8E`/`0x8F` mirror dispatch miss have concrete local fixes. The post-RDRAM guard fixed the first `rdram+0x80800002` SIGBUS but not the later `rdram+0x80810002` idle crash. `LOD_ENABLE_FAST_GAMEPLAY_INPUT_SCRIPT` now reaches the idle target around `DL#972` and reproduced the crash quickly; active/current DMAMgr RAM was valid in that crash snapshot. Next iteration should add downstream PI DMA destination or graphics/scheduler evidence. Do not blindly widen the guard. The manual high-res `0x3880006d0` crash also shows base NI no-match handling is not fully classified. Keep debug-only input/CFB snapshots as test harnesses, avoid gamestate skips as permanent fixes, and keep audio disabled until the CPU/TLB/DMAMgr route is stable.
