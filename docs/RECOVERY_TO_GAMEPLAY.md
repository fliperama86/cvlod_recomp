...
- Added default-off `LOD_ENABLE_KSEG0_FAULT_TRACE` to capture RDRAM-relative fault offsets plus lightweight context snapshots for `DMAMgr_updatePendingFileLoad` and `func_80043768`. A 700s automated boot-input + KSEG0-trace run reached the same low-res idle signature (`gs=3`, `exec=0x38000000`, `ni=0x803241C0`, width 320), passed the user's previous DL/VI crash window, and timed out cleanly beyond DL#21000/VI#40000 without a crash. This makes the idle KSEG0 crash look nondeterministic or timing-sensitive; the trace is ready for the next reproduction.
...
2. Capture an actual `0x80810002`-family crash with `LOD_ENABLE_KSEG0_FAULT_TRACE=1` if it reproduces; the first 700s trace run did not crash, so avoid changing the guard model based on a non-reproduced run.
...
| `LOD_ENABLE_KSEG0_FAULT_TRACE` | off | Debug-only lightweight crash-context snapshot for post-RDRAM/KSEG0 faults; validated by 700s non-crashing trace run | Enable for the next manual/automated `0x80810002` repro attempt |
...
Work item `G6-001`: preserve the now-validated low-resolution natural route into gameplay, then stabilize long-idle gameplay and high-resolution intro/gameplay `gs=3`. The NI TLB skip-window bug and `0x8E`/`0x8F` mirror dispatch miss have concrete local fixes. The post-RDRAM guard fixed the first `rdram+0x80800002` SIGBUS but not the later user-reported `rdram+0x80810002` idle crash; however, a 700s automated KSEG0-trace route passed that crash window without reproducing it. The next KSEG0 step is to capture an actual repro with `LOD_ENABLE_KSEG0_FAULT_TRACE=1`, not to blindly widen the guard. The manual high-res `0x3880006d0` crash also shows base NI no-match handling is not fully classified. Keep debug-only input/CFB snapshots as test harnesses, avoid gamestate skips as permanent fixes, and keep audio disabled until the CPU/TLB/DMAMgr route is stable.
