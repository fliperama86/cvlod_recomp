...
3. Improve KSEG0/PI evidence before accepting a wider guard model: capture a native backtrace on the next default-guard fast-script crash and/or add targeted PI DMA destination logging to prove whether any path is writing/reading `0x80810000`/past-RDRAM addresses after DMAMgr itself reports valid active/current RAM.
4. Keep `LOD_POST_RDRAM_GUARD_SIZE=0x20000` as diagnostic evidence for now. Use it for A/B stability checks, but leave the default at `0x10000` until the path is classified as legitimate open-bus/end-of-RDRAM behavior rather than a bad pointer/resource.
...
Work item `G6-001`: preserve the now-validated low-resolution natural route into gameplay, then stabilize long-idle gameplay and high-resolution intro/gameplay `gs=3`. The NI TLB skip-window bug and `0x8E`/`0x8F` mirror dispatch miss have concrete local fixes. The post-RDRAM guard fixed the first `rdram+0x80800002` SIGBUS but the default 64KB guard did not cover the later `rdram+0x80810002` idle crash. `LOD_ENABLE_FAST_GAMEPLAY_INPUT_SCRIPT` now reaches the idle target around `DL#972` and reproduced the crash quickly; active/current DMAMgr RAM was valid in that crash snapshot. A diagnostic `LOD_POST_RDRAM_GUARD_SIZE=0x20000` fast run survived 180s, supporting a guard-width/open-bus hypothesis, but native stack or downstream PI DMA / graphics-scheduler evidence is still needed before accepting a wider baseline guard. Use fast-skip for KSEG0 idle loops; reserve full natural prologue runs for intro-stability checks. The manual high-res `0x3880006d0` crash also shows base NI no-match handling is not fully classified. Keep debug-only input/CFB snapshots as test harnesses, avoid gamestate skips as permanent fixes, and keep audio disabled until the CPU/TLB/DMAMgr route is stable.
