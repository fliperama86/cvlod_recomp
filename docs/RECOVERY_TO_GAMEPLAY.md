...
| RENDER-002 | targeted geometry/display-list diagnostics for skirt/lantern/black-model glitches | default build and `build-render-geomtrace` compiled on `diag/render-glitch-root-causes`; first manual trace exposed throttle bug | Added `LOD_ENABLE_RENDER_GEOM_TRACE`, default OFF. When enabled it emits throttled RT64 traces for suspicious/overlay-sourced sub-display-list calls (including invalid target dumps), vertex loads, matrix loads, black/overlay light loads, and BranchZ/W decisions. The trace build also uses `LOD_DISABLE_RT64_FORCE_BRANCH=ON` so manual tests can keep the better widescreen behavior while capturing model-glitch evidence. First manual run only emitted category limit messages because the helpers consumed trace budgets on non-emitted candidates; fixed by counting emitted suspicious/overlay logs only. | Manual repro should relaunch `build-render-geomtrace/LodRecomp`, route through the bad cutscenes, then inspect `[RT64-GEOM][DL]`, `[RT64-GEOM][DL-DUMP]`, `[RT64-GEOM][VTX]`, `[RT64-GEOM][MTX]`, `[RT64-GEOM][LIGHT]`, and `[RT64-GEOM][BRANCH*]` lines to classify whether the issue is a bad vertex source, bad matrix/skinning state, skipped sub-DL, or lighting/material state. |
