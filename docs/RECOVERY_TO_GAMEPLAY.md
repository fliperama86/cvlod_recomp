...
| `LOD_ENABLE_AUDIO_TRACE` | off | Debug-only app-layer audio telemetry: SDL device open, frequency changes, queued sample min/max/nonzero counts, and SDL queued byte count | Use with `LOD_OVERRIDE_ALENVMIXERPULL=0` to determine whether audio is blocked before AI submission, produces silence, or reaches SDL with nonzero samples |
...
| `LOD_ENABLE_AUDIO_PULL_TRACE` | off | Debug-only CPU libaudio pull-chain wrappers for source/envmixer/resample/ADPCM functions | Use when classifying whether voices/events generate alist commands before RSP execution |
| `LOD_ENABLE_RSP_AUDIO_TRACE` | off | Debug-only RSP audio task telemetry and first-task ucode/alist dumps to `/tmp/lod_aspMain_*.bin` | Use to generate/check the missing audio RSP recompilation; remove or keep default-off once RSP ucode is wired |
| `LOD_USE_RECOMPILED_AUDIO_RSP` | off | Selects experimental generated LoD audio RSP microcode instead of the timing-only stub | Required for any audio-output experiment; keep off until generated RSP audio is validated |
...
Parallel audio work is now past the first blocker: the generated audio RSP is wired, and the libaudio PVoice free-list bug was traced to empty `__osDequeueThread`/`__osEnqueueThread` stubs that must behave as ALLink list helpers. The remaining audio blocker is sparse/missing synthesized audio: short nonzero bursts reach `osAiSetNextBuffer`, `queue_samples`, and the raw SDL input stream, but most queued samples are still zero. Current RSP traces show plausible resample input collapsing to `-1..1` or near-zero around ENVMIXER/MIXER. Next focus is real progression: high-res/base-NI no-match `0x3880006d0` classification, longer gameplay/manual crash capture, and RSP audio envmixer/mixer fixed-point math comparison against expected ABI behavior. Use fast-skip for KSEG0 idle loops; reserve full natural prologue runs for intro-stability checks. Keep debug-only input/CFB/audio snapshots as test harnesses and avoid gamestate skips or app-side audio gain as permanent fixes.
