...
| `LOD_ENABLE_AUDIO_TRACE` | off | Debug-only app-layer audio telemetry: SDL device open, frequency changes, queued sample min/max/nonzero counts, and SDL queued byte count | Use with `LOD_OVERRIDE_FUNC_8009E480=0` to determine whether audio is blocked before AI submission, produces silence, or reaches SDL with nonzero samples |
...
| AUDIO-001 | `LOD_OVERRIDE_FUNC_8009E480=0`, no auto-input/traces | user manual | User reported no audible audio. | Disabling the audio-chain no-op is not sufficient; add app-layer audio telemetry to determine whether AI buffers are queued, whether samples are nonzero, and whether SDL receives data. |
| AUDIO-002 | `LOD_OVERRIDE_FUNC_8009E480=0`, `LOD_ENABLE_AUDIO_TRACE=1`, no auto-input | compile-only/manual-ready | Added default-off audio trace flag and built `build-audio-trace/LodRecomp` for manual testing. | Next manual result should include `[AUDIO]` lines to classify the remaining audio blocker. |
...
Work item `G6-001`: preserve the now-validated low-resolution natural route into gameplay, then stabilize long-idle gameplay and high-resolution intro/gameplay `gs=3`. The NI TLB skip-window bug and `0x8E`/`0x8F` mirror dispatch miss have concrete local fixes. The post-RDRAM guard fixed the first `rdram+0x80800002` SIGBUS but the default 64KB guard did not cover the later `rdram+0x80810002` idle crash. `LOD_ENABLE_FAST_GAMEPLAY_INPUT_SCRIPT` now reaches the idle target around `DL#972`; active/current DMAMgr RAM was valid in the reproduced crash snapshot. Telemetry is now good enough to move on: native backtrace, RDRAM-relative fault decoding, DMAMgr snapshot, last-`DMA_ROMCopy` snapshot, and enhanced NI no-match context are available for future crashes. Do not keep looping on the guard unless a new crash provides fresh evidence. Next focus is real progression: high-res/base-NI no-match `0x3880006d0` classification, longer gameplay/manual crash capture, and eventually override/audio cleanup. Use fast-skip for KSEG0 idle loops; reserve full natural prologue runs for intro-stability checks. Keep debug-only input/CFB snapshots as test harnesses and avoid gamestate skips as permanent fixes.
