...
2. Stop using full natural prologue as the default KSEG0 repro path. Keep it only for intro-stability checks; for idle/KSEG0 work, add/use a faster skip-to-gameplay harness that reaches the same `gs=3 exec=0x38000000 ni=0x803241C0` idle section quickly.
3. Improve KSEG0/PI evidence before changing the guard model: capture the active/current `DMAMgr_updatePendingFileLoad` fields and, if needed, add targeted PI DMA destination logging to prove whether `do_rom_read` is writing to `0x80810000`/past-RDRAM addresses.
...
Work item `G6-001`: preserve the now-validated low-resolution natural route into gameplay, then stabilize long-idle gameplay and high-resolution intro/gameplay `gs=3`. The NI TLB skip-window bug and `0x8E`/`0x8F` mirror dispatch miss have concrete local fixes. The post-RDRAM guard fixed the first `rdram+0x80800002` SIGBUS but not the later `rdram+0x80810002` idle crash. A reduced DMAMgr-only KSEG0 trace captured that crash once, but a follow-up trace run passed the same window and timed out, so the failure is nondeterministic/timing-sensitive. Next iteration should use a faster skip-to-gameplay harness for KSEG0/idle work, then add PI DMA destination evidence if active/current DMAMgr fields stay valid before the crash. Do not blindly widen the guard. The manual high-res `0x3880006d0` crash also shows base NI no-match handling is not fully classified. Keep debug-only input/CFB snapshots as test harnesses, avoid gamestate skips as permanent fixes, and keep audio disabled until the CPU/TLB/DMAMgr route is stable.
