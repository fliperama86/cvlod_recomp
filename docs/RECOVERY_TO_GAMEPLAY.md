...
- Windows issue #7/#21 crackle evidence: reporter logs used SDL `directsound` with a 44.1 kHz/8-channel default device while LoD requested 48 kHz/F32/stereo. The Windows baseline now selects SDL `wasapi` at process entry when `SDL_AUDIODRIVER` is missing, empty, or explicitly `directsound`, while preserving other explicit driver overrides for diagnostics. SUPERNOVA smoke build/run logged `SDL audio driver: wasapi`, 48 kHz stereo default/output, and no stale process; issue #21 still needs reporter confirmation that crackle is fixed again.
