...
Setup
-----
1. Extract this entire folder (keep the DLLs next to LodRecomp.exe).
2. Place your legally dumped stock Castlevania: Legacy of Darkness ROM
   in the same folder, named:  rom.z64
   (If no ROM is found, the app will ask you to select one and remember it.)
3. Run LodRecomp.exe.

Windows SmartScreen
-------------------
This build is not yet code-signed, so the first launch may show
"Windows protected your PC". Click "More info" then "Run anyway".
You can verify your download against the SHA256 checksum published
...
Config files and the current-session log are stored in %APPDATA%\LodRecomp.
For crash reports, attach %APPDATA%\LodRecomp\LodRecomp.log. It is
truncated on each launch and keeps the most recent 512 KiB by default.
Use LOD_LOG_MAX_KB=4096 only when asked for a larger debug log.
